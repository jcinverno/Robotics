...
public class SimResourceLibrary implements IResource {
    ...
    public boolean executeSkill(String skillID) {
        sim.simxSetStringSignal(clientID, myAgent.getLocalName(), new CharWA(skillID), remoteApi.simx_opmode_blocking);
        IntW opRes = new IntW(-1);
        long startTime = System.currentTimeMillis();
        while ((opRes.getValue() != 1) && (System.currentTimeMillis() - startTime < timeout)) {
            sim.simxGetIntegerSignal(clientID, myAgent.getLocalName(), opRes, remoteApi.simx_opmode_blocking);
            try {
                Thread.sleep(100);
        ...
        sim.simxClearIntegerSignal(clientID, myAgent.getLocalName(), remoteApi.simx_opmode_blocking);
