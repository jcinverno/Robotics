...
public class SimTransportLibrary implements ITransport {
    ...
    public boolean executeMove(String origin, String destination, String productID) {
        sim.simxSetStringSignal(clientID, "Move", new CharWA(productID + "#" + origin + "#" + destination), remoteApi.simx_opmode_blocking);
        IntW opRes = new IntW(-1);
        long startTime = System.currentTimeMillis();
        while ((opRes.getValue() != 1) && (System.currentTimeMillis() - startTime < timeout)) {
            sim.simxGetIntegerSignal(clientID, "Move", opRes, remoteApi.simx_opmode_blocking);
            try {
                Thread.sleep(100);
        ...
        sim.simxClearIntegerSignal(clientID, "Move", remoteApi.simx_opmode_blocking);
